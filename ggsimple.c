	...
	for (t = 0; t < n_seq; ++t) {
		...
		for (i = 0; i < gt->n_gc; ++i) {
			...
			for (j = 0; j < n_ss; ++j) {
				...
				if (k <= le) continue;
				if ((pd>(I.coff[1]-I.coff[0])?pd-(I.coff[1]-I.coff[0]):(I.coff[1]-I.coff[0])-pd) < (opt->min_var_len>(I.coff[1]-I.coff[0])/10?opt->min_var_len:(I.coff[1]-I.coff[0])/10)) { // if length difference > min_var_len, just insert
					int32_t qd = I.coff[1] - I.coff[0], mlen, blen, score;
					l_pseq = mg_path2seq(km, g, gt, ls, le, I.voff, &pseq, &m_pseq);
					score = mg_wfa_cmp(km, l_pseq, pseq, qd, &seq[t].seq[I.coff[0]], 5000, 0, &mlen, &blen);
					if (score > 0) {
						...
						score = mg_wfa_cmp(km, l_pseq, pseq, qd, &seq[t].seq[I.coff[0]], ((int)(qd*opt->ggs_inv_cap_frac)>5000?(int)(qd*opt->ggs_inv_cap_frac):5000), opt->ggs_inv_step, &mlen, &blen);
